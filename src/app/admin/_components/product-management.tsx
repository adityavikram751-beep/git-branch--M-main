...
export function ProductManagement() {
  ...
  // Flag to ensure all products are fetched only once per session
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}?page=${currentPage}&limit=100`, {
        headers: getHeaders(),
      });
  ...
  // ---------- Fetch ALL products (once, parallel) ----------
  const fetchAllProductsOnce = useCallback(async () => {
    if (allProductsFetchedRef.current && allProducts.length > 0) {
      return;
    }
    setIsLoadingAll(true);
    try {
      ...
      if (!firstRes.ok) return;
      const firstData: ApiResponse = await firstRes.json();
      const totalPagesAll = firstData.totalPages;
      const firstPageProducts = firstData.products.map(mapApiProduct);
      ...
      setAllProducts(allFetched);
      allProductsFetchedRef.current = true;
      console.log(`✅ All ${allFetched.length} products fetched once`);
    } catch (err) {
      console.error("Error fetching all products:", err);
      toast.error("Failed to load all products for search/filter");
    } finally {
      setIsLoadingAll(false);
    }
  }, [allProducts.length]);
  ...
  const handleRefresh = useCallback(() => {
    ...
    setIsSelectAllGlobal(false);
    fetchProducts();
    toast.success("Refreshed current page");
  }, [fetchProducts]);
  ...
  // Search/filter effect: if filter active, trigger fetching all products once
  useEffect(() => {
    if (search.trim() !== "" || statusFilter !== "all") {
      fetchAllProductsOnce();
    }
  }, [search, statusFilter, fetchAllProductsOnce]);
  ...
  // ---------- filteredProducts definition (MUST be before useEffect that uses it) ----------
  const filteredProducts = useMemo(() => {
    if (hasFilter) {
    ...
    return [...products];
  }, [products, allProducts, search, statusFilter, sortOption, hasFilter]);

  // Selection status effect — must come AFTER filteredProducts definition
  useEffect(() => {
    if (filteredProducts.length > 0) {
      const allSelected = filteredProducts.every((p) =>
        selectedProducts.has(p.id)
  ...
  // ---------- Business Logic ----------
  const handleAddProduct = (_newProduct: Product) => {
    allProductsFetchedRef.current = false;
    setAllProducts([]);
    setRefreshTrigger((prev) => prev + 1);
    toast.success("Product added successfully! Refreshing...");
  };
  ...
  const handleSelectAllGlobal = () => {
    if (allProducts.length === 0) {
      toast.info("Please perform a search or filter first to load all products.");
      return;
    }
    if (isSelectAllGlobal) {
      setSelectedProducts(new Set());
  ...
  return (
    <div className="p-6 space-y-6">
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-rose-600">
            ...
            {allProducts.length > 0 && !hasFilter && (
          ...
          {selectedProducts.size > 0 && (
            <div className="flex gap-2 mr-4">
              ...
              <Button
                ...
                title={
                  allProducts.length === 0
                    ? "Type in search or select a filter to load all products"
                    : ""
                }
              >
                {isSelectAllGlobal ? (
      ...
      {isLoading && products.length === 0 && !hasFilter && (
        <div className="flex justify-center items-center py-8">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-rose-700"></div>
        </div>
      )}
      ...
      <Card className="border-rose-200 bg-white/70 backdrop-blur-sm">
        ...
        <CardContent>
          <div className="w-full border border-rose-200 rounded-md overflow-hidden">
            ...
            <div className="max-h-[520px] overflow-y-auto">
              {isLoading && products.length === 0 && !hasFilter ? (
                ...
                <div className="text-center text-rose-700 py-10">
                  {hasFilter && allProducts.length === 0 && isLoadingAll
                    ? "Loading all products for search/filter (once only)..."
                    : hasFilter
                    ? "No products found matching your criteria."
                    : "No products found."}
                </div>
          ...
          {selectedProducts.size > 0 && (
            <div className="text-center text-rose-700 font-medium text-sm mt-4">
              ⚡ {selectedProducts.size} product(s) selected — Ready for bulk actions!
            </div>
          )}
        </CardContent>
      </Card>
